...
describe UpdateNewsEventRepresenter do
  ...
  describe "#to_json" do
    it "should export to json" do
      @news_event.extend(UpdateNewsEventRepresenter)
      result = @news_event.to_json
      result.should eq("{\"news_event\":{\"description\":\"#{@news_event.description}\",\"url\":\"#{@news_event.url}\",\"source\":\"#{@news_event.source}\",\"posted\":\"#{@news_event.posted}\"}}")
    end
  end

  describe "#from_json" do
    it "should import from json payload" do
      json = "{\"news_event\":{\"description\":\"#{@news_event.description}\",\"url\":\"#{@news_event.url}\",\"source\":\"#{@news_event.source}\",\"posted\":\"#{@news_event.posted}\"}}"
      new_news_event = NewsEvent.new
      new_news_event.extend(UpdateNewsEventRepresenter)

...
require 'service/representer/stack_representer'
require 'service/representer/create_stack_representer'
...
describe StackApiApp do
  ...
  describe "POST /" do
    before :each do
      @account = FactoryGirl.create(:account)
      @template = "{\"resources\":[]}"
      @create_stack = FactoryGirl.build(:stack, :account_id=>@account.id.to_s, :template=>@template,
        :description=>"foo").extend(CreateStackRepresenter)
    ...
    it "should return the proper content type if data is missing" do
      @create_stack = FactoryGirl.build(:stack, :account_id=>@account.id, :template=>@template, :name=>nil).extend(CreateStackRepresenter)
      post "/", @create_stack.to_json
      last_response.headers["Content-Type"].should eq(JSON_CONTENT)
    end
    ...
    #   @create_stack = FactoryGirl.build(:stack, :account_id=>@account.id.to_s, :template=>@template, :name=>nil).extend(CreateStackRepresenter)
    #   post "/", @create_stack.to_json
    #   @template.published?.should eq(false)
  ...
  describe "PUT /:id" do
    before :each do
      @account = FactoryGirl.create(:account)
      @category = FactoryGirl.create(:category)
      @stack = FactoryGirl.create(:stack, :account=>@account, :category=>@category).extend(CreateStackRepresenter)

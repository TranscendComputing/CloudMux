...
class VCloudComputeApp < VCloudApp
  ...
  get '/organizations' do
    [OK, @compute.organizations.to_json]
  end
  ...
  post '/data_centers/:vdc_id/vapps/:id/clone' do
    if params['vapp_options']
      response = @compute.post_clone_vapp(
        params[:vdc_id],
        params['vapp_name'],
        params[:id],
        params['vapp_options']
  ...
  get '/data_centers/:vdc_id/vapps/:vapp_id/vms/:id/network' do
    network = @org.vdcs.get(params[:vdc_id])
                  .vapps.get(:vapp_id)
                  .vms.get(params[:id])
                  .network
    ...
    netobj = {
      ...
      :ip_address_allocation_mode => network.ip_address_allocation_mode
    }
  ...
  post '/data_centers/:vdc_id/vapps/:vapp_id/vms/:id/network' do
    ...
    unless params['is_connected'].nil?
      network.is_connected = params['is_connected']
    end

    unless params['ip_address_allocation_mode'].nil?
      network.is_address_allocation_mode = params['ip_address_allocation_mode']
    end

    unless params['mac_address'].nil?
  ...
  get '/data_centers/:vdc_id/vapps/:vapp_id/vms/:id/disks' do
    disks = @org.vdcs.get(params[:vdc_id])
                .vapps.get(params[:vapp_id])
                .vms.get(params[:id])
                .disks.all(false)

    disk_list = disks.find_all { |disk|
      disk.capacity_loaded?
    }.map { |disk|
  ...
  post '/data_centers/:vdc_id/vapps/:vapp_id/vms/:vm_id/disks' do
    response = @org.vdcs.get(params[:vdc_id])
                   .vapps.get(params[:vapp_id])
                   .vms.get(params[:vm_id])
                   .disks.create(params['size'])
    [OK, response.to_json]
  end
  ...
  put '/data_centers/:vdc_id/vapps/:vapp_id/vms/:vm_id/disks/:id' do
    disk = @org.vdcs.get(params[:vdc_id])
               .vapps.get(params[:vapp_id])
               .vms.get(params[:vm_id])
               .disks.get(params[:id].to_i)

    if params['capacity'].present? && params['capacity'].to_s != disk.capacity.to_s
      disk.capacity = params['capacity']
    end
  ...
  post '/data_centers/:vdc_id/vapps/:vapp_id/vms/:vm_id/disks/:id/attach' do
    if params['disk_options']
      response = @compute.post_attach_disk(
        params[:vm_id],
        params[:id],
        params['disk_options']
      )

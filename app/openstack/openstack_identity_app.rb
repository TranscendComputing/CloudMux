...
class OpenstackIdentityApp < ResourceApiBase
  ...
  get '/users' do
    response = params[:tenant_id].nil? ?
      @identity.list_users.body["users"] :
      @identity.list_users(params[:tenant_id]).body["users"]
  ...
  get '/tenants' do
    response = params[:filters].nil? ?
      @identity.tenants :
      @identity.tenants.all(params[:filters])
  ...
  get '/tenants/:id/users/:user_id/roles' do
    user_roles = @identity.list_roles_for_user_on_tenant(
      params[:tenant_id],
      params[:id]
    ).body["roles"]
  ...
  post '/tenants/:id/users/:user_id/roles/:role_id' do
    response = @identity.add_user_to_tenant(
      params[:id],
      params[:user_id],
      params[:role_id]
    ).body

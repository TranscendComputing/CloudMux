...
class OpenstackComputeApp < ResourceApiBase
    ...
    before do
        if(params[:cred_id].nil? || ! Auth.validate(params[:cred_id],"Compute Service","action_os"))
            halt [BAD_REQUEST]
        else
    ...
    post '/instances' do
        ...
        if(json_body.nil? || ! Auth.validate(params[:cred_id],"Compute Service","create_instance_os",@compute.servers.length))
            [BAD_REQUEST]
        else
            begin

...
class AwsComputeApp < ResourceApiBase
  ...
  post '/instances' do
    ...
    can_create_instance(
      ...
      "options" => {
        :instance_count => @compute.tags.all(
          :value => Auth.find_account(params[:cred_id]).login,
          :key   => "UserName",
    ...
    auth_validated = Auth.validate(
      params[:cred_id],
      "Elastic Compute Cloud",
      "create_vpc_instance",
      {:params => params, :instance => json_body["instance"]}
    )

    halt [BAD_REQUEST] unless auth_validated
    ...
    Auth.validate(
      ...
      "Elastic Compute Cloud",
      "create_auto_tags",
      {:params => params, :resource_id => response.id}
    )
  ...
  get '/availability_zones' do
    filters = params[:filters]
    response = filters.nil? ?
      @compute.describe_availability_zones.body["availabilityZoneInfo"] :
      @compute.describe_availability_zones(filters).body["availabilityZoneInfo"]
    [OK, response.to_json]
  end
  ...
  get '/security_groups' do
    begin
      ...
      response = filters.nil? ?
        ...
        @compute.security_groups.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  put '/security_groups/:id/add_rule' do
    ...
    group.authorize_port_range(
      rule["fromPort"].to_i..rule["toPort"].to_i,
      {
  ...
  post '/key_pairs/import' do
    ...
    if body["name"].nil?
      halt [BAD_REQUEST, "The necessary parameter 'name' was not specified."]
    elsif body["public_key"].nil?
      halt [BAD_REQUEST, "The necessary parameter 'public_key' was not specified."]
    end

    response = @compute.import_key_pair(body["name"], body["public_key"])
    [OK, "Successfully imported keypair."]
  end
  ...
  get '/spot_requests' do
    begin
      filters = params[:filters]
      response = filters.nil? ?
        @compute.spot_requests :
        @compute.spot_requests.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  get '/spot_prices' do
    filters = params[:filters]
    rersponse = filters.nil? ?
      @compute.describe_spot_price_history.body["spotPriceHistorySet"] :
      @compute.describe_spot_price_history(filters).body["spotPriceHistorySet"]
    [OK, response.to_json]
  end
  ...
  post '/spot_prices/current' do
    ...
    response = @compute.describe_spot_price_history(filters)
                       .body["spotPriceHistorySet"].first
    [OK, response.to_json]
  end
  ...
  get '/reserved_instances' do
    begin
      filters = params[:filters]
      response = filters.nil? ?
        @compute.describe_reserved_instances.body["reservedInstanceSet"] :
        @compute.describe_reserved_instances(filters).body["reservedInstanceSet"]
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  get '/reserved_instances/describe_offerings' do
    filters = params[:filters]
    response = filters.nil? ?
      @compute.describe_reserved_instances_offerings
              .body["reservedInstancesOfferingsSet"] :
      @compute.describe_reserved_instances_offerings(filters)
              .body["reservedInstancesOfferingsSet"]
    [OK, response.to_json]
  end
  ...
  post '/reserved_instances' do
    reserved_set = @compute.describe_reserved_instances_offerings
                           .body["reservedInstancesOfferingsSet"]
    
    option = reserved_set.nil? ? 0 : reserved_set.length
    json_body = body_to_json_or_die("body" => request)

    can_create_instance(
      "cred_id" => params[:cred_id],
      "action"  => "create_reserved",
      "options" => {
        :instance_count => @compute.tags.all(
          :value => Auth.find_account(params[:cred_id]).login,
          :key => "UserName",
          "resource-type" => "reserved-instance"
        ).length
      }
    )

    response = @compute.purchase_reserved_instances_offering(
      json_body["reserved_instances_offering_id"],
      json_body["instance_count"]
    )
  ...
  get '/vpcs' do
    begin
      filters  = params[:filters]
      response = filters.nil? ? @compute.vpcs : @compute.vpcs.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  post '/vpcs' do
    json_body = body_to_json_or_die("body" => request)
    options = json_body["vpc"]["InstanceTenancy"].nil? ?
      {} : {"InstanceTenancy" => json_body["vpc"]["InstanceTenancy"]}
  ...
  get '/dhcp_options' do
    begin
      filters = params[:filters]
      response = filters.nil? ? 
        @compute.dhcp_options :
        @compute.dhcp_options.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  get '/internet_gateways' do
    begin
      filters = params[:filters]
      response = filters.nil? ?
        @compute.internet_gateways :
        @compute.internet_gateways.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  post '/internet_gateways' do
    ...
    if Auth.validate(
        ...
        "Elastic Compute Cloud",
        "modify_gateway",
        {gw_type:"admin_internet_gateway",permissions: permissions}
      )
  ...
  post '/vpcs/:id/internet_gateways/:internet_gateway_id/attach' do
    response = @compute.attach_internet_gateway(
      params[:internet_gateway_id],
      params[:id]
    )
    [OK, response.to_json]
  end
  ...
  post '/vpcs/:id/internet_gateways/:internet_gateway_id/detach' do
    response = @compute.detach_internet_gateway(
      params[:internet_gateway_id],
      params[:id]
    )
    [OK, response.to_json]
  ...
  get '/route_tables' do
    begin
      filters = params[:filters]
      response = filters.nil? ?
        @compute.route_tables :
        @compute.route_tables.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  get '/network_acls' do
    begin
      filters = params[:filters]
      response = filters.nil? ?
        @compute.network_acls :
        @compute.network_acls.all(filters)
      [OK, response.to_json]
    rescue => error
      pre_handle_error(@compute, error)
    end
  ...
  get '/images' do
    ...
    options  = options_hash[params[:platform]] or default_options
  ...
  get '/tags' do
    filters  = params[:filters]
    response = filters.nil? ? @compute.tags : @compute.tags.all(filters)
    [OK, response.to_json]
  end
end
